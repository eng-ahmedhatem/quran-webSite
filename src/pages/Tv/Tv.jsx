...
export default function Tv() {
  ...
  function handelClick(card,e) {
    ...
    document.querySelectorAll('.tv .tv-content .cards .card').forEach(ele=> ele.classList.remove("active"))
  ...
  return (
    <div className="tv">
      ...
      <div className="tv-content">
        ...
        {server &&
          <iframe src={server} title="🔴 بث مباشر قناة السنة النبوية " frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"  allowfullscreen></iframe>

          }
        </div>
        <div className="cards">
          {data.map((ele) => (
            <div key={ele.id} onClick={(e) => handelClick(ele,e)} className="card">
              <span>{ele.name}</span>
            </div>
          ))}
